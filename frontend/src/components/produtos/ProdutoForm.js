...
const ProdutoForm = () => {
  ...
  const handleChange = (e) => {
    ...
    if (['preco_compra', 'preco_venda', 'quantidade_estoque', 'icms'].includes(name)) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (produto.preco_venda <= 0) {
      setMessage({ type: 'danger', text: 'O preço de venda deve ser maior que zero.' });
      return;
    }
    ...
    try {
      ...
      setMessage({ 
        type: 'danger', 
        text: err.response?.data?.error || 'Erro ao salvar o produto. Verifique os dados informados.' 
      });
    }
  };
  ...
  if (loading) return <div className="text-center p-5">Carregando dados do produto...</div>;
  if (error) return <div className="alert alert-danger">{error}</div>;

  return (
    <div>
      <div className="page-header">
        <h1>{isEditMode ? `Editar Produto: ${produto.nome}` : 'Novo Produto'}</h1>
      </div>
        ...
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            <Row>
              ...
              <Col md={4}>
                <Form.Group className="mb-3">
                  <Form.Label className="required-field">Preço de Compra (R$)</Form.Label>
                  <Form.Control
                    type="number"
                    name="preco_compra"
              ...
              <Col md={4}>
                <Form.Group className="mb-3">
                  <Form.Label className="required-field">Preço de Venda (R$)</Form.Label>
                  <Form.Control
                    type="number"
                    name="preco_venda"
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label className="required-field">Quantidade em Estoque</Form.Label>
                  <Form.Control
                    type="number"
