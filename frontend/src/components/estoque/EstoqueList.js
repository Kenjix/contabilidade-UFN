...
import { faPlus, faSearch, faBoxOpen, faArrowRight, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
const EstoqueList = () => {
  ...
  const getTipoBadge = (tipo) => {
    switch (tipo) {
      case 'entrada':
        return <Badge bg="success"><FontAwesomeIcon icon={faArrowRight} className="me-1" /> Entrada</Badge>;
      case 'saida':
        return <Badge bg="danger"><FontAwesomeIcon icon={faArrowLeft} className="me-1" /> Saída</Badge>;
      default:
        return <Badge bg="secondary">{tipo}</Badge>;
    }
  };

  if (loading) return <div className="text-center p-5">Carregando dados de estoque...</div>;
      ...
      <Card className="mb-4">
        <Card.Header className="bg-primary text-white">
          <div className="d-flex justify-content-between align-items-center">
            <h5 className="mb-0">
              {activeTab === 'produtos' ? 'Produtos em Estoque' : 'Movimentações de Estoque'}
            </h5>
            <InputGroup className="w-50">
              <Form.Control
                placeholder={`Buscar ${activeTab === 'produtos' ? 'produtos' : 'movimentações'}...`}
                value={searchTerm}
          ...
          {filteredItems.length === 0 ? (
            <div className="text-center p-4">
              <p className="text-muted">
                {searchTerm ? `Nenhum ${activeTab === 'produtos' ? 'produto' : 'movimentação'} encontrado com os critérios de busca.` : 
                               `Nenhum ${activeTab === 'produtos' ? 'produto' : 'movimentação'} cadastrado.`}
              </p>
              {!searchTerm && activeTab === 'movimentacoes' && (
                <Link to="/estoque/movimentacao" className="btn btn-primary">Registrar Nova Movimentação</Link>
              )}
            </div>
          ) : (
            activeTab === 'produtos' ? (
                ...
                <tbody>
                  {filteredItems.map((produto) => (
                    <tr key={produto.id}>
                      ...
                      <td className="text-end">R$ {(produto.quantidade_estoque * produto.preco_compra).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </Table>
                ...
                <tbody>
                  {filteredItems.map((movimentacao) => (
                    <tr key={movimentacao.id}>
                      <td>{new Date(movimentacao.data_movimentacao).toLocaleString()}</td>
                      <td>{movimentacao.produto.nome}</td>
                      <td className="text-center">{getTipoBadge(movimentacao.tipo)}</td>
